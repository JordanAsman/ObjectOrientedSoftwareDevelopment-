...
* The color of the trunk is <code>brown = new Color(145, 112,
...
1. In the SunComponent's paintComponent method, create a Sun using the
   default (parameterless) constructor, then draw it to the component by
...
2. Modify SunComponent's paintComponent method to create multiple little Suns (use NUM_LITTLE_SUMS for the exact number), and draw them to the screen
    ...
    * Use a for loop to do this! Don't hard-code each sun individually!
    * It should appear as the item at the bottom of the figure.


		
3. For your final submission, make sure your <code>paintComponent()</code> do the following so that we can check that it looks like the final sun picture:
